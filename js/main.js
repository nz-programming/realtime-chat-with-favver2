...
const toggleFavorite = (e) => {
  ...
  console.log('toggleFavoriteから渡されたmessageId',messageId)
  ...
  firebase
    ...
    console.log('if文の中のdbdata.favorites',dbdata.favorites)

    console.log('作成');
    console.log('作成後',dbdata.favorites)
  firebase
    .database()
    .ref(`favorites/${currentUID}/${messageId}`)
    .set({
...
const createMessageDiv = (messageId, message) => {
  ...
  console.log('mflクリックのmessageId',messageId)
  console.log('mflクリックのmessage',message)
  // TODO: お気に入りONのとき、お気に入りリンクのアイコンを 塗りつぶしあり(fa-star) に設定する

  if(dbdata.favorites[messageId]){
    divTag.find('.message__favorite-icon').attr('class','message__favorite-icon fa fa-star');
    console.log(divTag.find('.message__favorite-icon'));
  }
  return divTag;
};
...
const loadChatView = () => {
  ...
  favoritesRef.on('child_removed', (favSnapshot) => {
    ...
    $(`#message-id-${messageId} > .message__body > .message__info > .message__favorite-link > .fa-star`).attr('class','message__favorite-icon fa fa-star-o');
  ...
  favoritesRef.on('child_added', (favSnapshot) => {
    ...
    dbdata.favorites[messageId] = messageId;

    console.log('favSnapshot.keyの値',favSnapshot.key)
    console.log('★★dbdata.favorites[messageId]の値',dbdata.favorites[messageId])
    ...
    $(`#message-id-${messageId} > .message__body > .message__info > .message__favorite-link > .fa-star-o`).attr('class','message__favorite-icon fa fa-star');
